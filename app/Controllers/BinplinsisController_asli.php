...
class BinplinsisController extends BaseController
{
    ...
    public function nilaimental(): string
    {
        ...
        $minggu_aktif = (int) ($request->getGet('minggu') ?? 1);
        ...
        //---------------------------------------------------
        // 5. MAP DATA NILAI MENTAL (DIFILTER BERDASARKAN STATUS DANTON UNTUK DANKI)
        //---------------------------------------------------
        $map_nilai = [];
        $status_simpan = [];
        ...
        //---------------------------------------------------
        // 6. PEJABAT STRUKTUR TANDA TANGAN (DIOPTIMALKAN UNTUK SEMUA ROLE)
        //---------------------------------------------------
        $danton = null;
        $danki  = null;
        $danyon = null;
        ...
        // 1. Cari berdasarkan pleton aktif yang sedang dipilih/dideteksi
        ...
        // 2. Jika masih kosong dan login sebagai Danton, ambil langsung datanya sendiri
        ...
        // 3. Jika masih kosong dan login sebagai Danki, ambil Danton pertama di kompinya
        ...
        // 4. Fallback terakhir via data siswa pertama yang tampil di tabel
        ...
        // 1. Jika login sebagai Danki, ambil langsung datanya sendiri beserta pangkat
        if ($userLevel === 'danki') {
        ...
        // 2. Jika berdasarkan pleton aktif
        ...
        // 3. Fallback jika Danki masih kosong (misal diakses danton/danyon/admin tapi pletonAll)
        ...
        // 1. Jika login sebagai Danyon, ambil langsung datanya sendiri beserta pangkat
        if ($userLevel === 'danyon') {
        ...
        // 2. Jika login sebagai Danki, cari Danyon melalui relasi batalyon dari kompi Danki tersebut
        ...
        // 3. Jika berdasarkan pleton aktif
        else if (!empty($pletonDeteksi) && $pletonDeteksi !== 'All') {
            $danyon = $db->table('pegawai')
                ->select('pegawai.*, pangkat.nama_pangkat')
                ->join('batalyon', 'batalyon.danyon_id = pegawai.nomor_induk')
        ...
        // 4. Fallback mutlak terakhir jika Danyon masih kosong (ambil data pegawai pertama yang memimpin batalyon di sistem)
        if (empty($danyon)) {
            $danyon = $db->table('pegawai')
                ->select('pegawai.*, pangkat.nama_pangkat')
                ->join('batalyon', 'batalyon.danyon_id = pegawai.nomor_induk')
        ...
        if (!empty($map_nilai)) {
            ...
            $status_approval['status_danyon'] = $first_nilai['status_danyon'] ?? '0'; // Sesuaikan jika ada kolom status_danyon
        }
        ...
        return view('binplinsis/nilaimental', [
            ...
            'jumlah_notif'      => ($userLevel != 'siswa') ? $this->getJumlahNotifDanki() : 0,

            // Variabel Profil Pejabat Beserta Pangkatnya
            'danton'            => $danton,
            'danki'             => $danki,
            'danyon'            => $danyon,
    ...
    public function simpanNilaiMental()
    {
        try {
            ...
            if (!$siswa_id || !$minggu_ke || !is_array($skor)) {
                return $this->response->setJSON([
                    'success' => false,
            ...
            $builder = db_connect()->table('penilaian_mental');
            ...
            if (db_connect()->error()['code'] != 0) {
                return $this->response->setJSON(db_connect()->error());
            }
