...
function Save-Text([string]$Path, [string]$Text) { $Text | Set-Content -Encoding UTF8 $Path }
...
function Save-CollectorError([string]$ReportDir, [string]$Name, [string]$Message) { Save-Text (Join-Path $ReportDir $Name) $Message }
...
if ($running -notcontains $phpContainer) { throw "PHP container is not running: $phpContainer" }
...
if ($poolResult.ExitCode -ne 0) { throw "Unable to locate PHP-FPM [www] pool configuration. Exit $($poolResult.ExitCode):`n$($poolResult.Output)" }
...
# Send USR2 from Docker Engine/host. The PHP image does not need a kill executable.
$reloadOutput = docker kill --signal USR2 $phpContainer 2>&1 | Out-String
$reloadExitCode = $LASTEXITCODE
if ($reloadExitCode -ne 0) {
    Invoke-DockerExec @($phpContainer, 'cp', $backupPath, $poolPath) | Out-Null
    throw "PHP-FPM reload failed (exit code $reloadExitCode):`n$($reloadOutput.TrimEnd())"
}
...
try {
    ...
    if ($latestReport) {
        ...
        try { Save-ContainerFile $slowlogPath (Join-Path $reportDir 'php-fpm-slowlog.txt') | Out-Null } catch { Save-CollectorError $reportDir 'collector-error-slowlog.txt' $_.Exception.Message }
        try { Save-ContainerFile $poolPath (Join-Path $reportDir 'php-fpm-pool-config-after.txt') | Out-Null } catch { Save-CollectorError $reportDir 'collector-error-pool.txt' $_.Exception.Message }
        ...
        try {
            ...
            foreach ($procPath in ($pidList.Output -split "`r?`n")) {
                ...
                $fields = $statText.Substring($closeParen + 2).Trim() -split '\s+'
        ...
        } catch { Save-CollectorError $reportDir 'collector-error-pid.txt' $_.Exception.Message }
        ...
        } catch { Save-CollectorError $reportDir 'collector-error-runtime.txt' $_.Exception.Message }
        ...
        } catch { Save-CollectorError $reportDir 'collector-error-container-log.txt' $_.Exception.Message }
        ...
        } catch { Save-CollectorError $reportDir 'collector-error-php-stats.txt' $_.Exception.Message }
        ...
        if (@(docker ps --format '{{.Names}}' 2>$null) -contains $redisName) {
            try { $redisStats = docker stats $redisName --no-stream --format 'name={{.Name}} cpu={{.CPUPerc}} mem={{.MemUsage}} net={{.NetIO}} block={{.BlockIO}} pids={{.PIDs}}' 2>&1 | Out-String; Save-Text (Join-Path $reportDir 'redis-final-stats.txt') $redisStats.TrimEnd() } catch { Save-CollectorError $reportDir 'collector-error-redis-stats.txt' $_.Exception.Message }
            try { $redisInfo = docker exec $redisName redis-cli INFO stats 2>&1 | Out-String; Save-Text (Join-Path $reportDir 'redis-info-stats.txt') $redisInfo.TrimEnd() } catch { Save-CollectorError $reportDir 'collector-error-redis-info.txt' $_.Exception.Message }
        }
        if (@(docker ps --format '{{.Names}}' 2>$null) -contains 'e-ujian-mysql') {
            try { $mysqlStats = docker stats 'e-ujian-mysql' --no-stream --format 'name={{.Name}} cpu={{.CPUPerc}} mem={{.MemUsage}} net={{.NetIO}} block={{.BlockIO}} pids={{.PIDs}}' 2>&1 | Out-String; Save-Text (Join-Path $reportDir 'mysql-final-stats.txt') $mysqlStats.TrimEnd() } catch { Save-CollectorError $reportDir 'collector-error-mysql-stats.txt' $_.Exception.Message }
        }

        $slowlogLocal = Join-Path $reportDir 'php-fpm-slowlog.txt'
        $slowlogLines = if (Test-Path $slowlogLocal) { @(Get-Content $slowlogLocal) } else { @() }
        [ordered]@{ generated_at=(Get-Date).ToUniversalTime().ToString('o'); php_container=$phpContainer; pool_config=$poolPath; slowlog_timeout=$slowlogTimeout; slowlog_path=$slowlogPath; slowlog_line_count=$slowlogLines.Count; ptrace_denied=@($slowlogLines | Where-Object { $_ -match 'ptrace|Operation not permitted' }).Count; runner_exit_code=$runnerExit } | ConvertTo-Json | Set-Content -Encoding UTF8 (Join-Path $reportDir 'php-fpm-slowlog.json')
    }
    ...
    try {
        ...
        $restoreReloadOutput = docker kill --signal USR2 $phpContainer 2>&1 | Out-String
        $restoreReloadExitCode = $LASTEXITCODE
        if ($restoreReloadExitCode -ne 0) { throw "reload exit code $restoreReloadExitCode`: $($restoreReloadOutput.TrimEnd())" }
