...
Write-Host ' E-UJIAN LOGIN DIAGNOSTIC RUNNER V4' -ForegroundColor Cyan
Write-Host '============================================================' -ForegroundColor Cyan
...
# Known-good path from this Docker image. No find/grep/shell discovery.
$poolPath = '/usr/local/etc/php-fpm.d/docker.conf'
$poolCheck = Invoke-Docker @($phpContainer, 'test', '-f', $poolPath)
...
$backupPath = '/tmp/e-ujian-v4-pool-backup.conf'
$slowlogPath = '/tmp/e-ujian-php-fpm-slowlog-v4.log'
...
if (-not $original) { throw "Could not read $poolPath" }

$clean = $original -replace '(?ms)^; E-UJIAN-V4-SLOWLOG-BEGIN.*?^; E-UJIAN-V4-SLOWLOG-END\s*', ''
$patch = "`n; E-UJIAN-V4-SLOWLOG-BEGIN`nrequest_slowlog_timeout = $slowlogTimeout`nslowlog = $slowlogPath`nrequest_slowlog_trace_depth = 30`n; E-UJIAN-V4-SLOWLOG-END`n"
$newConfig = $clean.TrimEnd() + $patch

$hostConfig = Join-Path $env:TEMP "e-ujian-v4-$([guid]::NewGuid().ToString('N')).conf"
$newConfig | Set-Content -Path $hostConfig -Encoding UTF8
try {
    $install = Invoke-Docker @($phpContainer, 'cp', $hostConfig, $poolPath)
...
# Truncate slowlog without shell syntax: copy a zero-byte host file into the container.
$emptyLog = Join-Path $env:TEMP "e-ujian-v4-empty-$([guid]::NewGuid().ToString('N')).log"
New-Item -ItemType File -Path $emptyLog -Force | Out-Null
try {
    $clear = Invoke-Docker @($phpContainer, 'cp', $emptyLog, $slowlogPath)
...
$validation = Capture 'php-fpm-validation-before.txt' @($phpContainer, 'php-fpm', '-tt')
...
try {
    ...
    # Direct Docker commands only. Parsing remains on Windows PowerShell.
    try { Capture 'php-fpm-slowlog.txt' @($phpContainer, 'cat', $slowlogPath) | Out-Null } catch { Capture-Error 'slowlog' $_.Exception.Message }
    try { Capture 'php-fpm-pool-config-after.txt' @($phpContainer, 'cat', $poolPath) | Out-Null } catch { Capture-Error 'pool' $_.Exception.Message }
    try { Capture 'php-version.txt' @($phpContainer, 'php', '-v') | Out-Null } catch { Capture-Error 'php-version' $_.Exception.Message }
    try { Capture 'php-fpm-version.txt' @($phpContainer, 'php-fpm', '-v') | Out-Null } catch { Capture-Error 'php-fpm-version' $_.Exception.Message }
    try { Capture 'php-fpm-validation-after.txt' @($phpContainer, 'php-fpm', '-tt') | Out-Null } catch { Capture-Error 'validation-after' $_.Exception.Message }
    try { Capture 'php-processes.txt' @($phpContainer, 'ps', '-eo', 'pid,ppid,state,etime,%cpu,%mem,rss,args') | Out-Null } catch { Capture-Error 'processes' $_.Exception.Message }
    try { Capture 'php-top.txt' @($phpContainer, 'top', '-b', '-n', '1') | Out-Null } catch { Capture-Error 'top' $_.Exception.Message }
