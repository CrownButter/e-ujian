...
export default function () {
  ...
  const loginResponse = http.get(`${BASE_URL}${LOGIN_PATH}`, {
    redirects: 0,
    tags: { request: 'login_page', username },
  });

  const csrf = extractCsrf(loginResponse.body || '');
  ...
  if (!loginOk || !csrf) {
    ...
    metricFailure(`login_page_failed status=${loginResponse.status}`, username);
    return;
  }
  ...
  const responsePreview = (authResponse.body || '').replace(/\s+/g, ' ').slice(0, 300);
  metricFailure(
...
export function handleSummary(data) {
  ...
  const totalRequests = count('login_attempts') + count('auth_attempts');

  const report = {
    test: {
      name: 'E-UJIAN preprod direct login diagnostic',
      base_url: BASE_URL,
      configured_vus: VUS,
